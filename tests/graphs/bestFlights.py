...
def lcs(G,start, goal):
    explored = dict()
    frontier = []
    heapq.heappush(frontier,(0,0,0,[start]))
    while frontier:
        (pcost, flighttime, pstop, path) = heapq.heappop(frontier)
        state1 = path[-1]
        if state1 == goal:
            return path[1::2]
        for nstate in G[state1].keys():
            for flight in G[state1][nstate]:
                (cost,start,stop) = G[state1][nstate][flight]
                # only add if the start time is later than the current stop time
                if start > pstop:
                    total_cost = pcost + cost
                    if flighttime == 0:
                        newtime = (stop-start)
                    else:
                        newtime = flighttime + (stop-pstop)
                    path2 = path + [flight,nstate]
                    heapq.heappush(frontier,(total_cost,newtime,stop,path2))
    return None

def make_link(G,flight):
    (num,src,dst,start,stop,cost) = flight
    start = int(start.replace(':','',1))
    startmin = (start/100)*60 + start%100
    stop = int(stop.replace(':','',1))
    stopmin = (stop/100)*60 + stop%100

    if src not in G:
        G[src] = {}
    if dst not in G[src]:
        G[src][dst] = {}
    G[src][dst][num] = (cost,startmin,stopmin)
    return G
...
def find_best_flights(flights, origin, destination):
    ...
    for flight in flights:
        make_link(G,flight)
    best_flights = lcs(G,origin,destination)
    return best_flights
