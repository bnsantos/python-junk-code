...
def k_clique(G, k):
    ...
    for node in G:
        ...
        # First the node value is stored in x

        x=G[node]

        #Then G[node] is deleted

        del G[node]

        #Then with the 'new' graph we check the 'k_clique_decision'
        #If false, we restore the deleted node

        if not k_clique_decision(G, k): G[node]=x

        # if True we continue until we hit a graph with the same length
        # as k. The array or set containing the keys of this graph is the
        #SOLUTION

        if k_clique_decision(G, k) and len(G)==k: return [e for e in G.keys()]
